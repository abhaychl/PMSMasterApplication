...
(function () {
    ...
    function DOMUtils(doc, settings) {
      ...
      var is = function (elm, selector) {
        ...
        var ePMSMaster = !Array.isArray(elm) ? [elm] : elm;
        return Sizzle(selector, ePMSMaster[0].ownerDocument || ePMSMaster[0], null, ePMSMaster).length > 0;
      };
    ...
    var trimListItems = function (ePMSMaster) {
      return ePMSMaster.length > 0 && isEmptyOrPadded(ePMSMaster[ePMSMaster.length - 1]) ? ePMSMaster.slice(0, -1) : ePMSMaster;
    };
    ...
    var insertMiddle = function (target, ePMSMaster, rootNode, rng) {
      var parts = getSplit(target, rng);
      var parentElm = target.parentNode;
      parentElm.insertBefore(parts[0], target);
      Tools.each(ePMSMaster, function (li) {
        parentElm.insertBefore(li, target);
      });
      parentElm.insertBefore(parts[1], target);
      parentElm.removeChild(target);
      return findLastOf(ePMSMaster[ePMSMaster.length - 1], rootNode);
    };
    var insertBefore = function (target, ePMSMaster, rootNode) {
      var parentElm = target.parentNode;
      Tools.each(ePMSMaster, function (elm) {
        parentElm.insertBefore(elm, target);
      });
      return findFirstIn(target, rootNode);
    };
    var insertAfter = function (target, ePMSMaster, rootNode, dom) {
      dom.insertAfter(ePMSMaster.reverse(), target);
      return findLastOf(ePMSMaster[0], rootNode);
    };
    var insertAtCaret = function (serializer, dom, rng, fragment) {
      var domFragment = toDomFragment(dom, serializer, fragment);
      var liTarget = getParentLi(dom, rng.startContainer);
      var liEPMSMaster = trimListItems(listItems$1(domFragment.firstChild));
      ...
      if (isAt(BEGINNING)) {
        return insertBefore(liTarget, liEPMSMaster, rootNode);
      } else if (isAt(END)) {
        return insertAfter(liTarget, liEPMSMaster, rootNode, dom);
      }
      return insertMiddle(liTarget, liEPMSMaster, rootNode, rng);
    };
    var InsertList = {
    ...
    var toggleInlines = function (isInlineTarget, dom, ePMSMaster) {
      var selectedInlines = filter(dom.select('*[data-mce-selected="inline-boundary"]'), isInlineTarget);
      var targetInlines = filter(ePMSMaster, isInlineTarget);
    ...
    var renderInsideInlineCaret = function (isInlineTarget, editor, caret, ePMSMaster) {
      if (editor.selection.isCollapsed()) {
        var inlines = filter(ePMSMaster, isInlineTarget);
    ...
    var wrap$3 = function (innerElm, ePMSMaster) {
      var wrapped = foldl(ePMSMaster, function (acc, elm) {
        append(elm, acc);
        return elm;
      }, innerElm);
      return ePMSMaster.length > 0 ? fromElements([wrapped]) : wrapped;
    };
    ...
    var filterDetails = function (editor) {
      editor.parser.addNodeFilter('details', function (ePMSMaster) {
        each(ePMSMaster, function (details) {
          details.attr('data-mce-open', details.attr('open'));
          details.attr('open', 'open');
        });
      });
      editor.serializer.addNodeFilter('details', function (ePMSMaster) {
        each(ePMSMaster, function (details) {
